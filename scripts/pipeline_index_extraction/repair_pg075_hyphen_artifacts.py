# Usage: python scripts/pipeline_index_extraction/repair_pg075_hyphen_artifacts.py
# Repairs PG075 alphabetical payload entries split by OCR line-break hyphenation,
...
PAYLOAD = ROOT / "data/alphabetical_index_payloads/PG075_alphabetical_indices.json"
INTERMEDIATE = ROOT / "data/intermediate_payloads/PG075"
...
def merge_entry(left: dict[str, Any], right: dict[str, Any], merged_keys: list[str]) -> None:
    left_had_page = left.get("inferred_printed_page") not in (None, "")
    ...
    for field in ("section_start_file", "editorial_anchor_file", "heading_letter", "parent_node_key"):
        if left.get(field) in (None, "") and right.get(field) not in (None, ""):
            left[field] = right.get(field)

    if not left_had_page and right.get("inferred_printed_page") not in (None, ""):
        left["inferred_printed_page"] = right.get("inferred_printed_page")
        if right.get("target_file_best") not in (None, ""):
            left["target_file_best"] = right.get("target_file_best")
    elif left.get("target_file_best") in (None, "") and right.get("target_file_best") not in (None, ""):
        left["target_file_best"] = right.get("target_file_best")
    ...
    repair = raw_json.setdefault("pg075_rerun_linebreak_hyphen_repair", {})
    ...
    for file in [
        ...
        if file and file not in repair["ocr_evidence_files"]:
            repair["ocr_evidence_files"].append(file)
...
def clean_entry_fields(entry: dict[str, Any]) -> bool:
    ...
    if changed:
        entry.setdefault("raw_json", {})["pg075_rerun_linebreak_hyphen_repair"] = {
...
def clean_ref_fields(ref: dict[str, Any]) -> bool:
    ...
    for field in ("ref_raw", "page_ref_raw", "line_ref_raw", "range_start_raw", "range_end_raw"):
        ...
        if new != old:
            ...
            changed = True
    if changed:
        ref.setdefault("raw_json", {})["pg075_rerun_linebreak_hyphen_repair"] = {
...
def main() -> None:
    ...
    for ref in refs:
        ...
        if old_key in remap:
            ...
            ref.setdefault("raw_json", {})["pg075_rerun_entry_key_remapped_from"] = old_key
    ...
    coverage["entries_status_reason"] = (
        "Rerun repaired PG075 OCR line-break hyphen artifacts by merging split continuation "
        "entries and remapping their references while preserving the verified section structure."
    )
    coverage["evidence_files"] = [
        str(ROOT / "teste/PG075/text/9ee7850e-68d1-4119-a828-291d4eb4d63f-748.txt"),
        str(ROOT / "teste/PG075/text/9ee7850e-68d1-4119-a828-291d4eb4d63f-752.txt"),
        str(ROOT / "teste/PG075/text/9ee7850e-68d1-4119-a828-291d4eb4d63f-753.txt"),
    ]
    ...
    notes.append(
        {
            ...
            "date": now,
            "message": "Repaired PG075 validation-blocking OCR line-break hyphen artifacts and remapped continuation refs.",
    ...
    todo = {
        "volume_id": "PG075",
        "updated_at": now,
        "current_focus": "Payload repaired after PG075 hyphen-artifact validation failure.",
        "completed": [
            "Read skill contract and output format",
            "Inspected prior payload failure objects and representative OCR pages 748, 752, and 753",
        ...
        "blocked": [],
        "notes": [
            "Representative OCR confirms split forms such as generis, spiritum, transgressionis, and ORDO RERUM continuations.",
