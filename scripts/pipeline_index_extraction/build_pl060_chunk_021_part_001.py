#!/usr/bin/env python3
"""Build PL060 chunk section_021_part_001 from the bounded OCR file."""
...
OUTPUT_PATH = PROJECT_ROOT / "data/intermediate_payloads/PL060/chunks/section_021_part_001.json"
CHUNK_ID = "PL060:chunk:021:001"
TOP_SECTION_ID = "PL060:candidate-section:021"
SECTION_KEY = TOP_SECTION_ID
SECTION_FILE = PROJECT_ROOT / "teste/PL060/text/7b581ab9-9247-465f-8153-307a7d4f29ff-546.txt"
SECTION_FILE_START = str(SECTION_FILE)
ADJACENT_P_FILE = str(PROJECT_ROOT / "teste/PL060/text/7b581ab9-9247-465f-8153-307a7d4f29ff-544.txt")
...
SUSPICIOUS_BOOKS = {"1", "11", "m", "I I", "I,I", "I.I", "n", "n.", "u", "ui", "v", "vii"}
...
GROUP_PATTERN = re.compile(
    r"(?<![A-Za-zÀ-ÿÆæŒœ])"
    r"(?P<book>III|VII|II|UI|I\s*,\s*I|I\s*\.\s*I|I\s+I|11|1|I|S|M|N\.?|U|V)"
    r"\s*[.,]\s*"
...
def main() -> None:
    ...
    section = {
        ...
        "section_order": 21,
        "section_kind": "concordance_index",
        "heading_raw": "1033 INDICES IN DRACONTIUM. 1064",
        ...
        "confidence": 0.92,
        "raw_json": {
            "section_kind_reason": "The owned spread keeps the generic running head `INDICES IN DRACONTIUM`, but the adjacent files 545 and 547 print the explicit `INDEX VERBORUM ET PHRAISUM` title and all owned lines use the same Dracontius internal I/II/III/S locator system, so this page remains part of the verbal concordance.",
            "header_raw": header_546,
            ...
            "numbering_inference": "The owned header misreads the left editorial page as `1033`, but the surrounding sequence 1061-1062 on file 545, 1065-1066 on file 547, and 1067-1068 on file 548 establishes that the owned spread must be editorial pages 1063-1064.",
            "inspection_evidence": {
                "542_550_header_window_checked": True,
                "544_547_body_window_checked": True,
                "owned_file_body_checked_via_read_ocr_page_text": True,
                "raw_545_546_boundary_checked": True,
            },
            "section_scope_note": "Only entries beginning on owned physical file 546 were emitted. The first visible line `peccata, S. 304...` continues the final P-entry on file 545 and was therefore omitted here, while the remaining owned lines stay within the inherited `P` run and stop before the later `Q` marker on file 547.",
            "locator_system": "book_roman_plus_internal_number",
            "extra_investigation": {
                "files_checked": [
                    str(PROJECT_ROOT / "teste/PL060/text/7b581ab9-9247-465f-8153-307a7d4f29ff-550.txt"),
                ...
                "searches_checked": [
                    "header sequence review across physical files 542-550",
                    "raw boundary review of files 545-546 to confirm the opening carry-over line",
                ],
                "reason": "Used to correct the damaged header numbering, confirm the inherited P section from file 544, and avoid re-emitting the cross-page entry that begins on file 545."
            },
        },
    }
    ...
    ensure_letter_node(
        ...
        ADJACENT_P_FILE,
        80,
        "explicit_letter_marker_on_adjacent_page",
        inferred=True,
        reason="File 544 prints the explicit `P` marker, and files 545-547 show the same uninterrupted P run through the owned file 546.",
    )

    entries: list[dict] = []
    refs: list[dict] = []
    entry_order = 1
    current_letter: str | None = "P"

    for file_path in sorted(physical_files, key=lambda p: int(p.stem.rsplit("-", 1)[1])):
        _, records = parse_text_blocks(file_path)
        if records and str(records[0]["text"])[0].islower():
            records = records[1:]
    ...
    payload = {
        ...
        "section_id": TOP_SECTION_ID,
        "chunk_id": CHUNK_ID,
        "input_fingerprint": "f7f502b82860d76f39851ddd5f4a0ed1d49395f80aac0d2c695dbc73836858e5",
        "status": "complete",
        ...
        "scripture_refs": [],
        "notes": [
            "The owned file yields 166 non-empty OCR body lines; the opening lowercase line is a continuation from file 545 and was intentionally not re-emitted, leaving 149 serialized concordance entries after joining 10 wrapped multi-line entries.",
            "The generic running head on file 546 was retained in `heading_raw`, but the editorial spread was inferred as 1063-1064 from the surrounding 542-550 header sequence because the left page number is OCR-damaged as `1033`.",
            "Damaged book tokens such as `1`, `11`, `m`, `n`, `u`, and `ui` were preserved in `ref_raw`; their best local interpretations were recorded in `book_norm` only when the Dracontius concordance pattern made the mapping defensible."
        ],
    }
