...
from scripture_ref_normalizer import extract_citations_from_value_cached as _extract_citations_from_value
...
def test_empty_keywords_array_is_valid_payload():
    parsed, issue = normalize_keywords_payload('{"keywords": []}')

    assert parsed == {"keywords": []}
    assert issue is None


def test_parse_keywords_response_accepts_json_code_fence():
    parsed, _ = parse_keywords_response(
        '```json\n{"keywords_ranking": ["Trindade", "Graça"]}\n```'
    )

    assert parsed["keywords"] == ["Trindade", "Graça"]


def test_parse_judge_response_accepts_json_code_fence():
    decisions, issues = parse_judge_response(
        '```json\n{"terms": [{"original": "Graça", "decision": "keep"}]}\n```'
    )

    assert issues == []
    assert decisions[0]["original"] == "Graça"


def test_category_fallback_preserves_biblical_numeric_comma():
    categories = _parse_categories(
        "- Obras citadas: Lucas 24,39; Salmos 22,1, Evangelho de João"
    )

    assert categories["Obras citadas"] == [
        "Lucas 24,39",
        "Salmos 22,1",
        "Evangelho de João",
    ]
...
@pytest.mark.parametrize("text", ["Sabedoria cósmica", "Tito Lívio"])
def test_accented_word_after_book_name_is_not_a_roman_chapter(text):
    ...
    assert not [record for record in records if record.get("number") is not None]
...
def test_clean_keywords_structure_keeps_abbreviation_chapter_without_verse():
    ...
    # Agora a pipeline expande abreviações de capítulo para o nome canônico
    # quando existe uma âncora correspondente na categoria.
    assert cleaned["keywords"] == ["Gênesis 3"]
...
def test_clean_keywords_structure_normalizes_citations_in_both_obras_and_termos():
    """Citações bíblicas em obras_citadas E termos_tecnicos_lat_gr devem ser
    ambas normalizadas (vírgula em vez de dois-pontos, nome por extenso)."""
    parsed = {
        "keywords": ["Gn 3:22", "1 Cor 5:15"],
        "categorias": {
            "obras_citadas": ["Gn 3:22", "De Trinitate"],
            "termos_tecnicos_lat_gr": ["1 Cor 5:15", "phantasma"],
        },
    }
    cleaned, issues = clean_keywords_structure(parsed)

    assert cleaned["categorias"]["obras_citadas"] == ["Gênesis 3,22", "De Trinitate"]
    assert cleaned["categorias"]["termos_tecnicos_lat_gr"] == ["1 Coríntios 5,15", "phantasma"]
    # Keywords devem normalizar pois ambas têm âncora em categorias
    assert "Gênesis 3,22" in cleaned["keywords"]
    assert "1 Coríntios 5,15" in cleaned["keywords"]
    assert "normalized_bible_citation" in issues


def test_clean_keywords_structure_normalizes_citations_only_in_termos_tecnicos():
    """Citação bíblica presente APENAS em termos_tecnicos_lat_gr (sem obras_citadas)
    também deve ser normalizada."""
    parsed = {
        "keywords": ["1 Cor 5:15"],
        "categorias": {
            "termos_tecnicos_lat_gr": ["1 Cor 5:15", "phantasma"],
...
def test_new_abbrev_chapter_expands_when_category_book_only():
    parsed = {
        "keywords": ["Gn 3"],
        "categorias": {"obras_citadas": ["Gênesis"]},
    }
    cleaned, issues = clean_keywords_structure(parsed)
    # Agora esperamos que 'Gn 3' seja expandido para o nome canônico do livro
    assert cleaned["keywords"] == ["Gênesis 3"]
    assert "normalized_bible_citation" in issues


def test_new_abbrev_chapter_expands_for_latin_and_greek():
    # Latin abbreviation
    parsed_la = {
        "keywords": ["Gen 3"],
        "categorias": {"obras_citadas": ["Genesis"]},
    }
    cleaned_la, issues_la = clean_keywords_structure(parsed_la)
    assert cleaned_la["keywords"] == ["Gênesis 3"]
    assert "normalized_bible_citation" in issues_la

    # Greek form (nominative) or transliteration
    parsed_gr = {
        "keywords": ["Γένεσις 3"],
        "categorias": {"obras_citadas": ["Γένεσις"]},
    }
    cleaned_gr, issues_gr = clean_keywords_structure(parsed_gr)
    assert cleaned_gr["keywords"] == ["Gênesis 3"]
    assert "normalized_bible_citation" in issues_gr


def test_new_abbrev_with_verse_normalizes_even_with_book_anchor():
    parsed = {
        "keywords": ["Ps. CXII, 13"],
        "categorias": {"obras_citadas": ["Livro dos Salmos"]},
    }
    cleaned, issues = clean_keywords_structure(parsed)
    assert cleaned["keywords"] == ["Salmos 112,13"]
    assert cleaned["categorias"]["obras_citadas"] == ["Salmos"]
    assert "normalized_bible_citation" in issues


def test_new_evangelho_category_normalizes_keyword():
    parsed = {
        "keywords": ["João 14"],
        "categorias": {"obras_citadas": ["Evangelho de João (capítulo 14"]},
    }
    cleaned, issues = clean_keywords_structure(parsed)
    assert cleaned["keywords"] == ["João 14"]
    assert cleaned["categorias"]["obras_citadas"] == ["João 14"]
    assert "normalized_bible_citation" in issues
