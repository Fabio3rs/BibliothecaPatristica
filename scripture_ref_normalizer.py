...
# Memoized public wrapper for use by external callers. Keeps the internal
# implementation untouched and provides a simple cache to avoid re-parsing
# identical strings multiple times in batch operations.
...
def clean_keyword_original(text: str) -> str:
    ...
    # Não remover parênteses que fazem parte do título, ex.: "De mendacio (Agostinho)".
    strip_chars = " \"'«»“”‘’[]{}|\\/–—-:;.,!?·•*&"
...
def is_book_abbreviation_token(token: str) -> bool:
    """Heurística pública: determina se um initial token parece uma
    abreviação de livro (ex: 'Gn', 'Ps', 'Ap'). Usada pelo pipeline para
    regras conservadoras de normalização.
    """
    if not token:
        return False
    t = token.strip().rstrip(".")
    tnorm = _normalize(t)
    if tnorm in _BOOK_ABBREVIATIONS:
        return True
    # very short tokens are likely abbreviations (1-3 chars)
    if len(tnorm) <= 3:
        return True
    return False
...
def _build_index() -> None:
    # Incluir também as formas em grego (LIVROS_GR) para reconhecer nomes
    # escritos no alfabeto grego e normalizá-los para o nome canônico em PT.
    sources = [LIVROS_PT, LIVROS_LA, LIVROS_GR_TRANSLIT, LIVROS_GR]
    for src in sources:
        for i, name in enumerate(src):
            key = _normalize(name)
            if key not in _INDEX:
                _INDEX[key] = (INDEX_BOOK_LABELS[i], i)
    ...
    for alias, idx in aliases.items():
        ...
        if key not in _INDEX:
            _INDEX[key] = (INDEX_BOOK_LABELS[idx], idx)

    # Abreviações sigla
    for abbr, idx in _BOOK_ABBREVIATIONS.items():
        # adicionar a forma direta (sem espaços) se ainda não presente
        if abbr not in _INDEX:
            _INDEX[abbr] = (INDEX_BOOK_LABELS[idx], idx)
        # Para abreviações numeradas como '1cor', '2tm' inclua variantes com
        # espaço entre o ordinal e a sigla ('1 cor', '2 tm') e também a
        # variante romana ('i cor', 'ii tm') para melhorar detecção em textos.
        m = re.fullmatch(r"([123])(\w+)", abbr)
        if m:
            number, base = m.groups()
            spaced = f"{number} {base}"
            roman = _ORDINAL_ROMAN.get(number)
            if spaced not in _INDEX:
                _INDEX[spaced] = (INDEX_BOOK_LABELS[idx], idx)
            if roman:
                roman_variant = f"{roman} {base}"
                if roman_variant not in _INDEX:
                    _INDEX[roman_variant] = (INDEX_BOOK_LABELS[idx], idx)


# Build Aho-Corasick automaton for fast book-name detection on normalized text.
_AHO_AUTOMATON = None


def _ensure_aho():
    global _AHO_AUTOMATON
    if _AHO_AUTOMATON is not None:
        return
    A = ahocorasick.Automaton()
    # _INDEX keys are normalized forms -> (canon, idx)
    for key, (canon, idx) in _INDEX.items():
        if not key:
            continue
        A.add_word(key, (key, idx))
    A.make_automaton()
    _AHO_AUTOMATON = A


def extract_citations_from_value_ac(
    raw_value: str,
    *,
    source_kind: str,
    source_path: str,
    ...
    """Fast extractor using Aho-Corasick to find book names and a small
    deterministic token parser to obtain chapter/verse when present.
    This is designed as a faster fallback to the heavy regex-based extractor
    in workloads with many items.
    """
    ...
    _ensure_aho()
    matches = []  # list of (start_pos, key, idx)
    for end_idx, (key, idx) in _AHO_AUTOMATON.iter(norm):
        start_idx = end_idx - len(key) + 1
        matches.append((start_idx, end_idx, key, idx))

    if not matches:
        # no book names found; try title-only fallback
        return _extract_book_only_titles(cleaned, source_kind=source_kind, source_path=source_path, support_mode=support_mode)

    # If the whole cleaned string matches a title spec, prefer the title-only
    # normalization (this ensures category items like "Evangelho segundo Lucas"
    # or "Livro de Daniel" are expanded to canonical book names).
    title_only = _extract_book_only_titles(cleaned, source_kind=source_kind, source_path=source_path, support_mode=support_mode)
    if title_only:
        return title_only

    # Tokenize cleaned original to align and parse numbers nearby
    raw_tokens = re.split(r"\s+", cleaned)
    norm_tokens = [_normalize(t) for t in raw_tokens]

    # compute character start positions for each token in `cleaned`
    token_starts: list[int] = []
    pos = 0
    for i, t in enumerate(raw_tokens):
        # skip possible multiple spaces in original cleaned representation
        while pos < len(cleaned) and cleaned[pos].isspace():
            pos += 1
        token_starts.append(pos)
        pos += len(t)

    def find_ngram_index_and_endchar(key_norm: str) -> tuple[int | None, int | None, int | None]:
        kparts = key_norm.split()
        kn = len(kparts)
        for i in range(len(norm_tokens) - kn + 1):
            if " ".join(norm_tokens[i : i + kn]) == key_norm:
                # compute end char index immediately after the ngram in cleaned
                start_char = token_starts[i]
                end_token = i + kn - 1
                end_char = token_starts[end_token] + len(raw_tokens[end_token])
                return i, end_char, end_token
        return None, None, None

    results: list[dict] = []
    seen_spans = set()
    for start_idx, end_idx, key, idx in matches:
        token_pos, char_end, end_token = find_ngram_index_and_endchar(key)
        if token_pos is None:
            continue
        # If the matched ngram is embedded inside a larger token (unlikely),
        # skip it. We rely on token alignment above so this should be rare.
        raw_token = raw_tokens[end_token]
        # Conservative mode: when support_mode is False, avoid accepting
        # very short abbreviations (like 'Gn', 'Ap') unless they include a
        # trailing dot or punctuation, or appear in full form. This keeps
        # behavior compatible with the original conservative extractor.
        if not support_mode:
            key_is_abbrev = key in _BOOK_ABBREVIATIONS or len(key) <= 3
            if key_is_abbrev:
                # accept only if the original token has an explicit dot
                # (e.g. 'Ap.'), or the following character is punctuation
                next_ch = cleaned[char_end : char_end + 1]
                if not raw_token.endswith('.') and not re.match(r'[\.:,\(\)]', next_ch):
                    continue
        # substring immediately after the matched ngram (allow punctuation)
        look = cleaned[char_end : char_end + 60]
        main = None
        verse = None
        alt = None

        # Try several tolerant patterns in order of specificity.
        # 1) num[:.,]verse  e.g. 12:3  or 12,3 or 12.3
        # (?![A-Za-zÀ-ÿ]) garante que o numeral não seja a letra inicial de uma
        # palavra comum (ex: "Damasceno" → 'D' bloqueado).
        # O grupo num usa alternância para evitar mistura de caixa: um numeral
        # romano válido é todo maiúsculo ou todo minúsculo (ex: "Mill" = M+ill
        # mistura as duas caixas e NÃO deve casar).
        _NUM_PAT = r"(?P<num>[0-9]{1,6}|[IVXLCDM]{1,6}|[ivxlcdm]{1,6})"
        m = re.match(rf"^\s*[:\,\.]?\s*{_NUM_PAT}(?![A-Za-zÀ-ÿ])(?:\s*[:.,]\s*(?P<verse>\d{{1,3}}(?:-\d{{1,3}})?))?", look)
        match_obj = None
        if m:
            match_obj = m
            main = _parse_ref_number(m.group("num"))
            if m.group("verse"):
                verse = m.group("verse")
        else:
            # 2) pattern like "12 (11)" or "12 (11),"
            _NUM_PAT2 = r"(?P<num>[0-9]{1,4}|[IVXLCDM]{1,4}|[ivxlcdm]{1,4})"
            _ALT_PAT2 = r"(?P<alt>[0-9]{1,4}|[IVXLCDM]{1,4}|[ivxlcdm]{1,4})"
            m2 = re.match(rf"^\s*{_NUM_PAT2}(?![A-Za-zÀ-ÿ])\s*\(\s*{_ALT_PAT2}(?![A-Za-zÀ-ÿ])\s*\)", look)
            if m2:
                match_obj = m2
                main = _parse_ref_number(m2.group("num"))
                alt = _parse_ref_number(m2.group("alt"))
            else:
                # 3) maybe the number is attached with punctuation separated by a space, e.g. "Ps. CXII, 13"
                _NUM_PAT3 = r"(?P<num>[0-9]{1,6}|[IVXLCDM]{1,6}|[ivxlcdm]{1,6})"
                m3 = re.match(rf"^\s*[,\.]?\s*{_NUM_PAT3}(?![A-Za-zÀ-ÿ])\s*(?:,|:)\s*(?P<verse>\d{{1,3}}(?:-\d{{1,3}})?)", look)
                if m3:
                    match_obj = m3
                    main = _parse_ref_number(m3.group("num"))
                    verse = m3.group("verse")
                else:
                    # 4) tolerant pattern: allow roman numerals or digits followed by
                    # punctuation and an adjacent verse number within a slightly
                    # larger window (covers cases like "Ps. CXII, 13" where the
                    # book abbreviation consumes punctuation)
                    _NUM_PAT4 = r"(?P<num>[0-9]{1,6}|[IVXLCDM]{1,6}|[ivxlcdm]{1,6})"
                    _NONNUM4 = r"[^0-9IVXLCDMivxlcdm\n]"
                    m4 = re.match(rf"^\s*[:\,\.]?\s*{_NUM_PAT4}(?![A-Za-zÀ-ÿ])(?:{_NONNUM4}{{0,6}})(?P<verse>\d{{1,3}}(?:-\d{{1,3}})?)", look)
                    if m4:
                        match_obj = m4
                        main = _parse_ref_number(m4.group("num"))
                        verse = m4.group("verse")

        # construct record; use the matched substring as the raw fragment
        if match_obj:
            raw_frag = cleaned[token_starts[token_pos] : (char_end + match_obj.end())]
        else:
            # If the matched ngram spans the whole cleaned string, return the
            # full cleaned value as raw fragment so higher-level callers treat
            # the category item as a book-only anchor and can normalize it.
            if token_pos == 0 and char_end >= len(cleaned):
                raw_frag = cleaned
            else:
                raw_frag = cleaned[token_starts[token_pos] : char_end]
        ...
        if record:
            span_key = (idx, token_pos, main)
            if span_key in seen_spans:
                continue
            seen_spans.add(span_key)
            results.append((token_pos, record))
...
def keywords_cite_books(keywords: list[str]) -> list[tuple[str, str, int]]:
    ...
    for kw in keywords:
        ...
        if result:
            ...
            continue
        # 2. Tokeniza — remove pontuação de borda de cada token
        raw_tokens = kw.split()
        tokens = [re.sub(r"^[\W_]+|[\W_]+$", "", t) for t in raw_tokens]
        # Se houver um prefixo numérico ou romano seguido de uma sigla, combine-os
        # ex.: ['2', 'Tm', '2:19'] -> ['2 tm', '2:19'] para preservar o ordinal
        combined: list[str] = []
        i = 0
        while i < len(tokens):
            t = tokens[i]
            next_t = tokens[i + 1] if i + 1 < len(tokens) else None
            # numeral arábico puro (ex: '2') ou romano (ex: 'ii')
            if next_t and re.fullmatch(r"\d+", t):
                # combine with following alpha token
                if re.fullmatch(r"[A-Za-zÀ-ÿ]+", next_t):
                    combined.append(f"{t} {next_t}")
                    i += 2
                    continue
            if next_t and re.fullmatch(r"[IVXLCDMivxlcdm]+", t):
                if re.fullmatch(r"[A-Za-zÀ-ÿ]+", next_t):
                    combined.append(f"{t} {next_t}")
                    i += 2
                    continue
            combined.append(t)
            i += 1
        tokens = [t for t in combined if t]
        # filtra tokens que são números puros ou tokens de verso como '2:19'
...
# Labels usados para indexação (forma com algarismo arábico onde aplicável)
INDEX_BOOK_LABELS = [_index_book_label(name) for name in LIVROS_PT]


# Build the index now that INDEX_BOOK_LABELS is available
_build_index()
...
def _extract_citations_from_value(
    ...
    *,
    ...
    if not cleaned:
        return []
    # Prefer the fast Aho-Corasick based extractor which finds book-name
    # anchors and does a small deterministic parse for chapter/verse.
    # Fall back to the original regex-based explicit extractor when AC
    # yields nothing or raises an unexpected error.
    # Conservative mode (used for category anchors): prefer the explicit
    # regex-based extractor so we avoid over-eager normalization. For
    # support_mode (keywords), prefer the fast AC extractor.
    if not support_mode:
        ...
        if explicit:
            return explicit
        # fallback to AC/title-only
        try:
            ac_records = extract_citations_from_value_ac(
                raw_value,
                source_kind=source_kind,
                source_path=source_path,
...
def keywords_check(keywords_json: object, debug_label: str = "") -> bool:
    payload = _coerce_keywords_payload(keywords_json)
    categorias = payload.get("categorias")
    if not isinstance(categorias, dict):
        categorias = {}

    # Categorias que podem conter citações bíblicas usadas como âncoras
    _BIBLE_CATEGORY_TARGETS = {
        "obras_citadas", "obras", "works", "works_cited",
        "termos_tecnicos", "termos_tecnicos_lat_gr",
        "termos_técnicos", "termos tecnicos",
        "technical_terms",
    }

    bible_cat_keys: list[tuple[str, str]] = []  # (chave_original, chave_normalizada)
    for key, value in categorias.items():
        if not isinstance(value, list):
            continue
        norm_key = _normalize(str(key)).replace(" ", "_")
        if norm_key in _BIBLE_CATEGORY_TARGETS:
            bible_cat_keys.append((key, norm_key))

    # Coleta itens de todas as categorias que podem conter citações bíblicas
    all_obras_items: list[tuple[str, str, str]] = []  # (raw_value, source_kind, source_path)
    for cat_key, norm_key in bible_cat_keys:
        items = _get_category_items(keywords_json, cat_key)
        for idx, raw_value in enumerate(items):
            all_obras_items.append((raw_value, norm_key, f"categorias.{norm_key}[{idx}]"))
    ...
    for raw_value, source_kind, source_path in all_obras_items:
        for record in extract_citations_from_value_cached(
            raw_value,
            source_kind=source_kind,
            source_path=source_path,
            support_mode=False,
